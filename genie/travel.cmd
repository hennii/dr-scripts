...
###########################################
# CHANGELOG - Latest Update: 9/12/2019
...
if matchre("(el'|el'b|el'ba|el'bai|el'bain|el'bains)","%destination") then
     {
	      var detour el'bain
		  goto THERENGIA
	 }
...
if "$zoneid" = "60" && "$guild" = "Thief" then
          {
              if $Athletics.Ranks >= %undersegoltha then
                  {
                      gosub MOVE 107
                      if ("$zoneid" = "60") then gosub MOVE 107
                      gosub MOVE cross
                      pause 0.1
                      if ("$zoneid" = "1a") then gosub MOVE cross
                  }
...
ONFERRY:
  ...
  matchre OFFTHERIDE ^The warship lands with a creaky lurch
  matchwait
...
NOCOIN:
  ...
  if "$zoneid" = "61" then
   ...
   if "$zoneid" = "7" then
    ...
    if (("$zoneid" = "116") && matchre("(rossman|lang|theren|rakash|muspari|fornsted|el'bain|mriss|merk|hara)","%detour")) then
        {
